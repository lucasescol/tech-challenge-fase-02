...
public class RestauranteController {
        ...
        @PostMapping
        @Operation(summary = "Cadastrar novo restaurante", description = "Cria um novo restaurante no sistema com informações de endereço, tipo de cozinha e horário de funcionamento")
        @ApiResponses(value = {
                        ...
                        @ApiResponse(responseCode = "400", description = "Dados inválidos ou tipo de cozinha não permitido", content = @Content),
                        @ApiResponse(responseCode = "401", description = "Não autenticado", content = @Content)
        })
        public ResponseEntity<RestauranteResponseDTO> cadastrar(
                        @Parameter(description = "Dados do novo restaurante", required = true) @Valid @RequestBody NovoRestauranteDTO dto) {
        ...
        @GetMapping("/{id}")
        @Operation(summary = "Buscar restaurante por ID", description = "Retorna os detalhes completos de um restaurante específico pelo seu identificador")
        @ApiResponses(value = {
        ...
        public ResponseEntity<RestauranteResponseDTO> obterPorId(
                        @Parameter(description = "ID do restaurante", required = true, example = "1") @PathVariable Long id) {
        ...
        @GetMapping
        @Operation(summary = "Listar todos os restaurantes", description = "Retorna uma lista completa com todos os restaurantes cadastrados no sistema")
        @ApiResponses(value = {
        ...
        @PutMapping("/{id}")
        @Operation(summary = "Atualizar restaurante", description = "Atualiza todas as informações de um restaurante existente (nome, endereço, tipo de cozinha, horário)")
        @ApiResponses(value = {
        ...
        public ResponseEntity<RestauranteResponseDTO> atualizar(
                        @Parameter(description = "ID do restaurante", required = true, example = "1") @PathVariable Long id,
                        @Parameter(description = "Dados atualizados do restaurante", required = true) @Valid @RequestBody AtualizarRestauranteDTO dto) {
        ...
        @DeleteMapping("/{id}")
        @Operation(summary = "Deletar restaurante", description = "Remove permanentemente um restaurante do sistema. Esta operação não pode ser desfeita.")
        @ApiResponses(value = {
        ...
        public ResponseEntity<Void> deletar(
                        @Parameter(description = "ID do restaurante a ser deletado", required = true, example = "1") @PathVariable Long id) {
                var restaurante = obterRestaurantePorIdUseCase.execute(id);
                if (restaurante == null) {
                        return ResponseEntity.notFound().build();
                }
